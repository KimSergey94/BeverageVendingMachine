Платформа: .NET 5 (хотел совместить с другим тестовым заданием)
	...
	Сервис StorageService используется в качестве хранилища терминала и содержит напитки, монеты и все необходимые операции связанные с хранилищем.
	Сервис TerminalService используется в качестве терминала и содержит в себе хранилище IStorageService, информацию о выбранном товаре и имлементирует интерфейс ITerminalService. Позволяет взаимодействовать 
пользователям и админам с терминалом и его хранилищем
	Сервис AdminTerminalService используется для взаимодействия админов с терминалом
	Сервис UserTerminalService используется для взаимодействия пользователей с терминалом
	Сервис/Handler FileHandler используется для обработки и десериализации загруженных файлов

Выполненые необязательные требования:
